module mod_globals
  ...
  ! flat-plate geometry
  real(8), parameter :: x_in = 0.d0 * blt !-16.d0 * blt ! x(1); the inlet-shock threshold (Xsh - x_in)*tan(beta) depends on this
  ...
  real(8), parameter :: M0    = 5.92d0 !2.15
  !real(8), parameter :: p_tot = 100.d3 !25.d3 Re_x = 1.25 * 10**5 (Xsh = 100.d0 * blt)
  real(8), parameter :: p0    = 308.2 !p_tot / ((1.d0 + 0.5d0 * (gamma - 1.d0) * M0**2)**(gamma/(gamma-1.d0)))
  real(8), parameter :: T0    = 53.06d0 !288.15
  ...
  ! oblique shock
  real(8), parameter :: beta  = dacos(-1.d0) * 13.6d0 / 180.d0 !M0 = 2.15 theta6, 32.72!theta5, 31.83 !theta4, 30.96 !30.8_baseline
