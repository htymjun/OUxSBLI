module calc_time_dev
  ...
  use mpi
  use nvtx
  use mod_globals, only : id_rescale, id_exchange, id_visc, nt, np, nre2, rerank
  use calc_flux_base
  use calc_steps
  use calc_rescale
  use calc_para
...
contains
  ...
  subroutine allocate_device_mem(myrank, nx, ny, nz, dx, dy, dz, xix, etay, zetaz, Jacobian, over_Jacobian, ruvwp, T, mu, mut, qc2, E, F, G, Rv)
    use mod_globals, only : id_visc
    integer, intent(in)                       :: myrank, nx, ny, nz
    real(8), intent(out), allocatable, device :: dx(:), dy(:), dz(:), xix(:), etay(:), zetaz(:), Jacobian(:,:), over_Jacobian(:,:)
    real(8), intent(out), allocatable, device :: ruvwp(:,:,:,:), T(:,:,:), mu(:,:,:), mut(:,:,:), qc2(:,:,:)
    real(8), intent(out), allocatable, device :: E(:,:,:,:), F(:,:,:,:), G(:,:,:,:), Rv(:,:,:,:)
    integer ierr
    allocate(ruvwp(5,nx,ny,nz), E(5,nx-1,ny-2,nz-2), F(5,nx-2,ny-1,nz-2), G(5,nx-2,ny-2,nz-1), stat=ierr)
    allocate(dx(nx-1), dy(ny-1), dz(nz-1), xix(nx-1), etay(ny-1), zetaz(nz-1), Jacobian(nx,ny), over_Jacobian(nx,ny), stat=ierr)
    if (kind(id_visc) == 2) then
      allocate(T(nx,ny,nz), Rv(1,1,1,1), mu(1,1,1), mut(1,1,1), qc2(1,1,1), stat=ierr)
    elseif (kind(id_visc) == 4) then
      allocate(T(nx,ny,nz), Rv(4,nx,ny,nz), mu(nx,ny,nz), mut(1,1,1), qc2(1,1,1), stat=ierr)
    elseif (kind(id_visc) == 8) then
      allocate(T(nx,ny,nz), Rv(4,nx,ny,nz), mu(nx,ny,nz), mut(nx,ny,nz), qc2(nx,ny,nz), stat=ierr)
    endif
    if (ierr /= 0) then
  ...
  subroutine pre_calc(nx, ny, nz, myrank, nranks, x, dx_cpu, y, dy_cpu, z, dz_cpu, Jacobian_cpu, Q, overlap, &
                      dx, dy, dz, xix, etay, zetaz, Jacobian, over_Jacobian, QJ, ke0, entropy0)
    ...
    real(8), intent(out), device :: dx(nx-1), dy(ny-1), dz(nz-1), xix(nx-1), etay(ny-1), zetaz(nz-1)
    real(8), intent(out), device :: Jacobian(nx,ny), over_Jacobian(nx,ny)
    ...
    over_Jacobian = 1.d0 / Jacobian_cpu
    QJ = Q
    ! for multi GPU
  ...
  subroutine RungeKutta_3rd(id_RungeKutta, myrank, mygpu, nx, ny, nz, x, dx_cpu, y, dy_cpu, z, dz_cpu, Jacobian_cpu, Q)
    ...
    if (mod(myrank,2) == 0) then
      ...
      call allocate_device_mem(myrank, nx, ny, nz, dx, dy, dz, xix, etay, zetaz, Jacobian, over_Jacobian, ruvwp, T, mu, mut, qc2, E, F, G, Rv)
      ...
      do t1 = 1, nt
        ...
        if (mod(myrank,2) == 0) then
          ...
          if (kind(id_LL) == 4) then
            ...
            call calc_EFG(id_visc, nx, ny, nz, xix, etay, zetaz, Jacobian, over_Jacobian, QJ, ruvwp, T, mu, mut, qc2, E, F, G, Rv)
          endif
          !print *, "myrank is ", myrank, " calc EFG"
          call nvtxEndRange
          call nvtxStartRange("calc step", 2)
          call calc_step1<<<blocks,threads>>>(id_visc, nx, ny, nz, 1.d0, dx, dy, dz, E, F, G, Rv, QJ, QJ2)
        ...
        if (mod(myrank,2) == 0) then
          ...
          if (kind(id_LL) == 4) then
            ...
            call calc_EFG(id_visc, nx, ny, nz, xix, etay, zetaz, Jacobian, over_Jacobian, QJ2, ruvwp, T, mu, mut, qc2, E, F, G, Rv)
          endif
          call calc_step2<<<blocks,threads>>>(id_visc, nx, ny, nz, dx, dy, dz, E, F, G, Rv, QJ, QJ2)
          if (ndevices >= 2 .and. kind(id_exchange) == 4) then
            call exchange(id_rescale, myrank, nranks, overlap, nx, ny, nz, QJ2)
          endif
          if (kind(id_rescale) == 4) then
        ...
        if (mod(myrank,2) == 0) then
          ...
          call calc_step3<<<blocks,threads>>>(id_visc, nx, ny, nz, dx, dy, dz, E, F, G, Rv, QJ2, QJ)
          if (ndevices >= 2 .and. kind(id_exchange) == 4) then
    ...
    if (mod(myrank,2) == 0) then
      deallocate(ruvwp, T, mu, mut, qc2, QJ, QJ2, E, F, G, Rv, dx, dy, dz, xix, etay, zetaz, Jacobian, over_Jacobian)
      if (kind(id_LL) == 4) then
        deallocate(seed)
      endif
    endif
  ...
  subroutine RungeKutta_4th(id_RungeKutta, myrank, mygpu, nx, ny, nz, x, dx_cpu, y, dy_cpu, z, dz_cpu, Jacobian_cpu, Q)
    ...
    do t2 = 1, np
      do t1 = 1, nt
        ...
        if (mod(myrank,2) == 0) then
          ...
          call calc_step<<<blocks,threads>>>(id_visc, nx, ny, nz, 0.5d0, 1.d0, dx, dy, dz, E, F, G, Rv, QJ, QJs, Rs) ! QJs = Q2
          if (ndevices >= 2) then
        ...
        if (mod(myrank,2) == 0) then
          ...
          if (kind(id_LL) == 4) then
            ...
            call calc_EFG(id_visc, nx, ny, nz, xix, etay, zetaz, Jacobian, over_Jacobian, QJs, ruvwp, T, mu, mut, qc2, E, F, G, Rv)
          endif
          call calc_step<<<blocks,threads>>>(id_visc, nx, ny, nz, 0.5d0, 2.d0, dx, dy, dz, E, F, G, Rv, QJ, QJs, Rs) ! QJs = Q3
          if (ndevices >= 2) then
            call exchange(id_rescale, myrank, nranks, overlap, nx, ny, nz, QJs)
          endif
          if (kind(id_rescale) == 4) then
        ...
        if (mod(myrank,2) == 0) then
          ...
          call calc_step<<<blocks,threads>>>(id_visc, nx, ny, nz, 1.0d0, 2.d0, dx, dy, dz, E, F, G, Rv, QJ, QJs, Rs) ! QJs = Q4
          if (ndevices >= 2) then
            call exchange(id_rescale, myrank, nranks, overlap, nx, ny, nz, QJs)
          endif
          if (kind(id_rescale) == 4) then
        ...
        if (mod(myrank,2) == 0) then
          ...
          call calc_step4<<<blocks,threads>>>(id_visc, nx, ny, nz, dx, dy, dz, E, F, G, Rv, Rs, QJ)
          if (ndevices >= 2) then
            call exchange(id_rescale, myrank, nranks, overlap, nx, ny, nz, QJ)
          endif
          if (kind(id_rescale) == 4) then
    ...
    if (mod(myrank,2) == 0) then
      deallocate(ruvwp, T, mu, mut, qc2, QJ, QJs, Rs, E, F, G, Rv, dx, dy, dz, xix, etay, zetaz, Jacobian, over_Jacobian)
      if (kind(id_LL) == 4) then
        deallocate(seed)
      endif
    endif
  ...
  subroutine Gauss_RungeKutta(id_RungeKutta, myrank, mygpu, nx, ny, nz, x, dx_cpu, y, dy_cpu, z, dz_cpu, Jacobian_cpu, Q)
    ...
    real(8), allocatable, pinned :: Qm_cpu(:)
    ! GPU !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    real(8), allocatable, device :: ruvwp(:,:,:,:), QJ(:,:,:,:), QJs(:,:,:,:), E(:,:,:,:), F(:,:,:,:), G(:,:,:,:), Rv(:,:,:,:)
    real(8), allocatable, device :: T(:,:,:), mu(:,:,:), mut(:,:,:), qc2(:,:,:)
    ...
    do t2 = 1, np
      do t1 = 1, nt
        if (mod(myrank,2) == 0) then
          ! calc R1
          call calc_EFG(id_visc, nx, ny, nz, xix, etay, zetaz, Jacobian, over_Jacobian, QJ, ruvwp, T, mu, mut, qc2, E, F, G, Rv)
          call calc_step1<<<blocks,threads>>>(id_visc, nx, ny, nz, c1, dx, dy, dz, E, F, G, Rv, QJ, QJs)
          call set_bc(myrank, nx, ny, nz, Jacobian, QJs)
          call calc_EFG(id_visc, nx, ny, nz, xix, etay, zetaz, Jacobian, over_Jacobian, QJs, ruvwp, T, mu, mut, qc2, E, F, G, Rv)
          call calc_R<<<blocks,threads>>>(id_visc, nx, ny, nz, dx, dy, dz, E, F, G, Rv, R1)
          ! calc R2
          call calc_EFG(id_visc, nx, ny, nz, xix, etay, zetaz, Jacobian, over_Jacobian, QJ, ruvwp, T, mu, mut, qc2, E, F, G, Rv)
          call calc_step1<<<blocks,threads>>>(id_visc, nx, ny, nz, c2, dx, dy, dz, E, F, G, Rv, QJ, QJs)
          call set_bc(myrank, nx, ny, nz, Jacobian, QJs)
          call calc_EFG(id_visc, nx, ny, nz, xix, etay, zetaz, Jacobian, over_Jacobian, QJs, ruvwp, T, mu, mut, qc2, E, F, G, Rv)
          call calc_R<<<blocks,threads>>>(id_visc, nx, ny, nz, dx, dy, dz, E, F, G, Rv, R2)
          do itr = 1, max_itr
            ! calc R1
            call calc_Gauss_step<<<blocks,threads>>>(nx, ny, nz, a11, a12, R1, R2, QJ, QJs)
            call set_bc(myrank, nx, ny, nz, Jacobian, QJs)
            call calc_EFG(id_visc, nx, ny, nz, xix, etay, zetaz, Jacobian, over_Jacobian, QJs, ruvwp, T, mu, mut, qc2, E, F, G, Rv)
            call calc_R<<<blocks,threads>>>(id_visc, nx, ny, nz, dx, dy, dz, E, F, G, Rv, R1_new)
            ! calc R2
            call calc_Gauss_step<<<blocks,threads>>>(nx, ny, nz, a21, a22, R1, R2, QJ, QJs)
            call set_bc(myrank, nx, ny, nz, Jacobian, QJs)
            call calc_EFG(id_visc, nx, ny, nz, xix, etay, zetaz, Jacobian, over_Jacobian, QJs, ruvwp, T, mu, mut, qc2, E, F, G, Rv)
            call calc_R<<<blocks,threads>>>(id_visc, nx, ny, nz, dx, dy, dz, E, F, G, Rv, R2_new)
          ...
          call calc_Gauss_step_Q<<<blocks,threads>>>(nx, ny, nz, b1, b2, R1, R2, QJ)
          call set_bc(myrank, nx, ny, nz, Jacobian, QJ, Qre)
        endif
      enddo
      if (mod(myrank, 2) == 0) then
    ...
    if (mod(myrank,2) == 0) then
      deallocate(ruvwp, T, mu, mut, qc2, QJ, QJs, R1, R2, R1_new, R2_new, E, F, G, Rv, dx, dy, dz, xix, etay, zetaz, Jacobian, over_Jacobian)
      if (kind(id_LL) == 4) then
        deallocate(seed)
      endif
    endif
