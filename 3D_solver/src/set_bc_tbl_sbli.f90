  ...
  subroutine set_bc_Neumann_tbl_top_down(nx, ny, nz, offset, istart, iend, Jacobian, QJ)
    ...
    do k = 1+offset, nz-offset
      do i = istart, iend
        ...
        over_QJ1 = 1.d0 / QJ(1,i,ny-1,k)
        rhob = QJ(1,i,ny-1,k) * Jacobian(i,ny-1)
        ub   = QJ(2,i,ny-1,k) * over_QJ1
        vb   = QJ(3,i,ny-1,k) * over_QJ1
        wb   = QJ(4,i,ny-1,k) * over_QJ1
        pb   = gamma_1 * (QJ(5,i,ny-1,k) * Jacobian(i,ny-1) - 0.5d0 * rhob * (ub**2 + vb**2 + wb**2))
        QJ(1,i,ny,k) = rhob * Jacobian_tmp
        QJ(2,i,ny,k) = rhob * ub * Jacobian_tmp
        QJ(3,i,ny,k) = rhob * vb * Jacobian_tmp
        QJ(4,i,ny,k) = rhob * wb * Jacobian_tmp
        QJ(5,i,ny,k) = (pb * over_gamma_1 + 0.5d0 * rhob * (ub**2 + vb**2 + wb**2)) * Jacobian_tmp
        ! NoSlip
