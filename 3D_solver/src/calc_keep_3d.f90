  ...
  attributes(device) function KEEP4(id_accuracy, rho, u, v, w, uu, p, T, Normal) result(F)
    ...
    block
      ...
      F(4) = fma(-RV3, w1, fma(RV1_RV2, w2, fma(RV1_RV3, w3, fma(-RV2, w4, pres * Normal(4)))))
    end block
    block
      real(8) ene
      ! internal energy
      ...
      ene  = fma( RV1, u2*u3 + v2*v3 + w2*w3, &
             fma(-RV2, u2*u4 + v2*v4 + w2*w4, &
             fma(-RV3, u1*u3 + v1*v3 + w1*w3, ene)))
      ! pressure diffusion
      !F(5) = ene + (two_third * (uu(2)*p(3)+uu(3)*p(2)) &
      !          - one_twelfth * (uu(2)*p(4)+uu(4)*p(2) &
      !                          +uu(1)*p(3)+uu(3)*p(1)))
      F(5) = fma(two_third, uu(2)*p(3) + uu(3)*p(2), &
  ...
  attributes(device) function KEEP6(id_accuracy, rho, u, v, w, uu, p, T, Normal) result(F)
    ...
    block
      ...
      !ruw = RV6*w(1) + RV3_RV5*w(2) + RV1_RV2_RV4*w(3) + RV1_RV3_RV6*w(4) + RV2_RV5*w(5) + RV4*w(6) + pres * Normal(4)
      ruu = mom(u, pres * Normal(2), RV4, RV6, &
                RV2_RV5, RV3_RV5, RV1_RV2_RV4, RV1_RV3_RV6)
      ruv = mom(v, pres * Normal(3), RV4, RV6, &
                RV2_RV5, RV3_RV5, RV1_RV2_RV4, RV1_RV3_RV6)
      ruw = mom(w, pres * Normal(4), RV4, RV6, &
                RV2_RV5, RV3_RV5, RV1_RV2_RV4, RV1_RV3_RV6)
      F(2) = ruu
      F(3) = ruv
      F(4) = ruw
    ...
    block
      ...
      ene = fma( RV6, u(1)*u(4) + v(1)*v(4) + w(1)*w(4), ene)
      ene = fma( RV5, u(2)*u(5) + v(2)*v(5) + w(2)*w(5), ene)
      ene = fma( RV4, u(3)*u(6) + v(3)*v(6) + w(3)*w(6), ene)
      ene = fma(-RV3, u(2)*u(4) + v(2)*v(4) + w(2)*w(4), ene)
      ene = fma(-RV2, u(3)*u(5) + v(3)*v(5) + w(3)*w(5), ene)
      ene = fma( RV1, u(3)*u(4) + v(3)*v(4) + w(3)*w(4), ene)
      ! pressure diffusion
      ...
      F(5) = fma( 0.75d0, uu(3)*p(4) + uu(4)*p(3), &
