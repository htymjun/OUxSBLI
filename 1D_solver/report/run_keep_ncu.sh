#!/usr/bin/env bash
# KEEP-only Nsight Compute sweep for FP32/FP64 simultaneous-execution studies.
#
# Default scope is intentionally small:
#   modes      : seq, warp, and fused only when SASS differs from seq
#   KEEP_PREC  : fp64, fp32, df
#   disabled   : warp_fused, term, PRESS_PREC='fp32', shadow/quant
set -euo pipefail
...
KEEP-specific options:
  --measure-fused       always profile fused with ncu
...
run_keep_case() {
  ...
  ncu_run_case keep "${case_prefix}_seq" seq "$order" "$order" "$keep_prec" "$visc_prec" fp64 fp64 fp64 fp64

  if [ "$MEASURE_FUSED" -eq 1 ]; then
    ncu_run_case keep "${case_prefix}_fused" fused "$order" "$order" "$keep_prec" "$visc_prec" fp64 fp64 fp64 fp64
  ...
  ncu_run_case keep "${case_prefix}_warp" warp "$order" "$order" "$keep_prec" "$visc_prec" fp64 fp64 fp64 fp64
}
