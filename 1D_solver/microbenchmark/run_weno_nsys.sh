...
Options:
  ...
  --nrepeat N           inner kernel repeat count, default 1
  --repeat N            process-level repeats, default 1
  --modes "A B ..."     override benchmark mode list
...
The selected modes compare identical arithmetic in:
  var_*                 physical-variable split: FP64 WENO for selected
                        variables and FP32 WENO for the rest.
...
complete_row() {
  ...
  awk -F, -v m="$mode" -v r="$rep" 'NR > 1 && $1 == m && $2 == r && $NF !~ /FAIL/ { found = 1 } END { exit found ? 0 : 1 }' "$OUT/summary.csv"
}

for mode in "${MODES[@]}"; do
  for rep in $(seq 1 "$REPEAT"); do
    if complete_row "$mode" "$rep"; then
      echo "skip completed $mode repeat $rep" | tee -a "$OUT/progress.log"
      continue
    fi
    base="${mode}__r${rep}"
    ...
    echo "[$(date +%H:%M:%S)] nsys $mode repeat $rep/$REPEAT" | tee -a "$OUT/progress.log"
    printf '%s\t%s\t%s\t%s\t%s %s %s %s\n' "$mode" "$rep" "$NX" "$NREPEAT" "$EXE" "$mode" "$NX" "$NREPEAT" >> "$OUT/commands.tsv"
    set +e
    "$NSYS" profile --trace="$NSYS_TRACE" --sample=none --cpuctxsw=none --backtrace=none \
      --force-overwrite=true --output="$report_base" "$EXE" "$mode" "$NX" "$NREPEAT" > "$raw_log" 2>&1
    rc=$?
    set -e
    if [ "$rc" -ne 0 ]; then
      printf '%s,%s,%s,%s,weno_%s,0,0,NSYS_FAIL,,,,,,,,PROFILE_FAIL\n' "$mode" "$rep" "$NX" "$NREPEAT" "$mode" >> "$OUT/summary.csv"
    ...
    python3 "$SCRIPT_DIR/parse_weno_nsys.py" \
      --mode "$mode" --repeat "$rep" --nx "$NX" --nrepeat "$NREPEAT" \
      --kernel "weno_.*${mode%%_*}" \
